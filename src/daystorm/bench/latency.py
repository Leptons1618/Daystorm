...
def halve_unquantized(model: torch.nn.Module) -> torch.nn.Module:
    """Cast everything bitsandbytes did NOT quantize to fp16.

    Quantizing only the Linear layers leaves LayerNorm weights in fp32 while
    activations arrive as fp16, and torch raises "expected scalar type Half but
    found Float" deep inside the norm. Params4bit tensors are uint8 and are not
    float32, so this pass skips them without needing to know about them.
    """
    ...
    for name, buf in list(model.named_buffers()):
        if buf.dtype == torch.float32:
            ...
            setattr(owner, name.rsplit(".", 1)[-1], buf.half())
    return model
...
def available_configs(device: str) -> list[tuple[str, dict]]:
    ...
    if device.startswith("cuda"):
        ...
        # torch.cuda.is_bf16_supported() returns True on sm_75 because it counts
        # emulation. Turing has no bf16 tensor cores, so the row is measured but
        # labelled - "supported" here does not mean "fast".
        native_bf16 = torch.cuda.get_device_capability()[0] >= 8
        if torch.cuda.is_bf16_supported():
            configs.append(("bf16", {"dtype": torch.bfloat16, "native": native_bf16}))
        else:
            configs.append(("bf16", {"skip": "not supported on this device"}))
        try:
...
def main() -> int:
    ...
    for name, cfg in available_configs(args.device):
        ...
        try:
            if quant == "nf4":
                model = halve_unquantized(to_nf4(model)).to(args.device)
                dtype = torch.float16  # activations fp16; the weights are the 4-bit part
            else:
                model = model.to(args.device, dtype)
            if args.compile:
                model = torch.compile(model)
        except Exception as exc:  # noqa: BLE001 - a broken config must not kill the sweep
            print(f"{name:<10} failed to build: {type(exc).__name__}: {exc}")
            results["runs"][name] = {"error": f"{type(exc).__name__}: {exc}"}
            continue

        results["runs"][name] = {}
        if name == "bf16" and not cfg.get("native", True):
            print(f"{name:<10} NOTE: emulated on sm_"
                  f"{''.join(str(x) for x in torch.cuda.get_device_capability())}, "
                  "no native bf16 tensor cores")
        for bs in args.batch:
            ...
            try:
                ...
                print(f"{name:<10} batch {bs:<3} failed: {type(exc).__name__}: {exc}")
                results["runs"][name][f"batch{bs}"] = {"error": str(exc)}
                continue
            timing["batch"] = bs
